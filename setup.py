...
def get_extension_hybrid_ep_cpp():
    ...
    enable_multinode = os.getenv("HYBRID_EP_MULTINODE", "0").strip().lower() in {
        "1",
        "true",
        "t",
        "yes",
    ...
    use_nixl = os.getenv("USE_NIXL", "0").strip().lower() in {
        "1",
        "true",
        "t",
        "yes",
    ...
    if enable_multinode:
        ...
        print(
            f"Multinode enabled: use_nixl={use_nixl} (USE_NIXL={os.getenv('USE_NIXL', '0')})"
        )
        if use_nixl:
            ...
            include_dirs.extend(
                [nixl_include, nixl_gpu_include, os.path.join(ucx_home, "include")]
            )
            ...
            subprocess.run(
                ["git", "submodule", "update", "--init", "--recursive"], cwd=current_dir
            )
            subprocess.run(
                [
                    "make",
                    "-j",
                    "src.build",
                    f"NVCC_GENCODE={to_nvcc_gencode(os.environ['TORCH_CUDA_ARCH_LIST'])}",
                ],
                cwd=nccl_dir,
                check=True,
            )
            include_dirs.append(
                os.path.join(
                    nccl_dir, "src/transport/net_ib/gdaki/doca-gpunetio/include"
                )
            )
            include_dirs.append(os.path.join(rdma_core_dir, "include"))
            library_dirs.append(os.path.join(rdma_core_dir, "lib"))
            ...
            shutil.copytree(
                os.path.join(
                    nccl_dir, "build/obj/transport/net_ib/gdaki/doca-gpunetio"
                ),
...
if __name__ == "__main__":
    ...
    if build_native_extensions:
        ext_modules = [get_extension_deep_ep_cpp(), get_extension_hybrid_ep_cpp()]
